...
import 'package:charts_common/src/chart/bar/bar_label_decorator.dart'
    show BarLabelDecorator, BarLabelAnchor, BarLabelPlacement, BarLabelPosition, BarLabelVerticalPosition;
...
import 'package:charts_common/src/chart/common/processed_series.dart' show ImmutableSeries;
...
class FakeBarRendererElement implements ImmutableBarRendererElement<String> {
  ...
  FakeBarRendererElement(this.datum, this.bounds, this.labelAccessor, this.data, {this.measureFn}) {
  ...
  group('vertical bar chart', () {
    test('Paint labels with default settings', () {
      ...
      final barElements = [
        ...
        FakeBarRendererElement(
            'A', Rectangle(leftPositionA, topPositionA, barWidthA, 100 - topPositionA), (_) => 'LabelA', data),
        FakeBarRendererElement(
            'B', Rectangle(leftPositionB, topPositionB, barWidthB, 100 - topPositionB), (_) => 'LabelB', data),
        FakeBarRendererElement(
            'C', Rectangle(leftPositionC, topPositionC, barWidthC, 100 - topPositionC), (_) => 'LabelC', data)
      ];
      final decorator = BarLabelDecorator<String>();

      decorator.decorate(barElements, canvas, graphicsFactory,
          drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: true);

      final captured = verify(canvas.drawText(captureAny, captureAny, captureAny)).captured;
      ...
      expect(offsetY, equals(topPositionB - decorator.labelPadding - labelHeight));
      // Bar 'C' checks.
      ...
      expect(offsetY, equals(topPositionC - decorator.labelPadding - labelHeight));
    });
    ...
    test('LabelPosition.inside always paints inside the bar', () {
      final barElements = [
        ...
        // has a width of 8.
        FakeBarRendererElement('A', Rectangle(10, 80, 6, 20), (_) => 'LabelABC', ['A']),
      ];
      ...
      expect(captured[1], equals(9)); // left position + bar width / 2 - text width / 2
      expect(captured[2], equals(80)); // top position + label padding
    });

    test('LabelPosition.outside always paints outside the bar', () {
      final barElements = [
        FakeBarRendererElement('A', Rectangle(10, 80, 10, 20), (_) => 'LabelA', ['A']),
      ];
      ...
      expect(captured[0].measurement.verticalSliceWidth, equals(10));
      expect(captured[1], equals(12)); // left position + bar width / 2 - text width / 2
      expect(captured[2], equals(70)); // top position - label padding - text height
    });

    test('Outside label with new lines draws multiline labels', () {
      final barElements = [
        FakeBarRendererElement('A', Rectangle(10, 80, 10, 20), (_) => 'LabelA\n(50)', ['A']),
      ];
      ...
      expect(captured[4], equals(13)); // left position + bar width / 2 - text width / 2
      expect(captured[5], equals(70)); // top position - label padding - text height
    });
    ...
    test('Inside and outside label styles are applied', () {
      ...
      final barElements = [
        ...
        // width of 4.
        FakeBarRendererElement('A', Rectangle(10, 80, 10, 20), (_) => 'LabelA', data),
        FakeBarRendererElement('B', Rectangle(25, 80, 4, 20), (_) => 'LabelB', data)
      ];
      final insideColor = Color(r: 0, g: 0, b: 0);
      final outsideColor = Color(r: 255, g: 255, b: 255);
      final decorator = BarLabelDecorator<String>(
          labelPadding: 0,
          insideLabelStyleSpec: TextStyleSpec(fontSize: 10, fontFamily: 'insideFont', color: insideColor),
          outsideLabelStyleSpec: TextStyleSpec(fontSize: 8, fontFamily: 'outsideFont', color: outsideColor));
      ...
      expect(captured[4], equals(24)); // left position + bar width / 2 - text width / 2
      expect(captured[5], equals(72)); // top position - label padding - text height
    });
    ...
    group('Null and empty label scenarios', () {
      test('Skip label if label accessor does not exist', () {
        ...
        BarLabelDecorator<String>().decorate(barElements, canvas, graphicsFactory,
            drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: true);
    ...
    group('BarLabelPlacement.opposeAxisBaseline', () {
      ...
      test('Paints negative outside labels', () {
        final barElements = [
          FakeBarRendererElement('A', Rectangle(10, 80, 10, 20), (_) => 'LabelA', ['A'], measureFn: (_) => -1.0),
        ];
      ...
      test('Paints negative inside labels', () {
        ...
        expect(captured[0].measurement.verticalSliceWidth, equals(12));
        expect(captured[1], equals(12)); // left position + bar width / 2 - text width / 2
        expect(captured[2], equals(88)); // top position + bar height - text height
      });
    });
  });
  ...
  group('horizontal bar chart', () {
    test('Paint labels with default settings', () {
      ...
      final barElements = [
        ...
        // 'LabelB' would not fit inside the bar in auto setting because it has
        // width of 5.
        FakeBarRendererElement('A', Rectangle(0, 20, 50, 20), (_) => 'LabelA', data),
        FakeBarRendererElement('B', Rectangle(0, 70, 5, 20), (_) => 'LabelB', data)
      ];
      final decorator = BarLabelDecorator<String>();

      decorator.decorate(barElements, canvas, graphicsFactory,
          drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false);
      ...
      expect(captured[2], equals(30 - decorator.insideLabelStyleSpec.fontSize ~/ 2));
      // For bar 'B'.
      expect(captured[3].maxWidth, equals(200 - 5 - decorator.labelPadding * 2));
      expect(captured[3].textDirection, equals(TextDirection.ltr));
      expect(captured[4], equals(5 + decorator.labelPadding));
      expect(captured[5], equals(80 - decorator.outsideLabelStyleSpec.fontSize ~/ 2));
    });
    ...
    test('LabelPosition.auto paints inside bar if outside bar has less width', () {
      final barElements = [
        // 'LabelABC' would not fit inside the bar in auto setting because it
        // has a width of 8.
        FakeBarRendererElement('A', Rectangle(0, 0, 8, 20), (_) => 'LabelABC', ['A']),
      ];
      ...
      BarLabelDecorator<String>(
              ...
              insideLabelStyleSpec: TextStyleSpec(fontSize: 10))
          .decorate(barElements, canvas, graphicsFactory,
              drawBounds: smallDrawBounds, animationPercent: 1.0, renderingVertically: false);
    ...
    test('Do not paint labels if they do not fit', () {
      final barElements = [
        // 'LabelABC' would not fit inside the bar in auto setting because it
        // has a width of 8.
        FakeBarRendererElement('A', Rectangle(0, 0, 6, 20), (_) => 'LabelABC', ['A']),
      ];
    ...
    test('LabelPosition.outside always paints outside the bar', () {
      final barElements = [
        FakeBarRendererElement('A', Rectangle(0, 0, 10, 20), (_) => 'Label', ['A']),
      ];
    ...
    test('TextAnchor.end starts on the right most of bar', () {
      final barElements = [
        FakeBarRendererElement('A', Rectangle(0, 0, 10, 20), (_) => 'LabelA', ['A'])
      ];
    ...
    test('RTL TextAnchor.start starts on the right', () {
      ...
      BarLabelDecorator<String>(
              ...
              insideLabelStyleSpec: TextStyleSpec(fontSize: 10))
          .decorate(barElements, canvas, graphicsFactory,
              drawBounds: drawBounds, animationPercent: 1.0, renderingVertically: false, rtl: true);
    ...
    group('BarLabelPlacement.opposeAxisBaseline', () {
      test('Paints positive outside labels', () {
        ...
        expect(captured[2], equals(85)); // top position + bar height / 2 - text height / 2
      });
      ...
      test('Paints positive inside labels', () {
        ...
        expect(captured[0].measurement.verticalSliceWidth, equals(12));
        expect(captured[1], equals(10)); // left position
        expect(captured[2], equals(84)); // top position + bar height / 2 - text height / 2
      });
